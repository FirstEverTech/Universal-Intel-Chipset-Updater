...
$QuietMode = $false   # new flag for -quiet / -q
...
if ($Help) {
 ...
 DESCRIPTION
  Universal Intel Chipset Device Updater is an advanced, security-focused 
  tool that automatically detects your Intel hardware and installs the
  latest official chipset INF files with enterprise-grade safety measures.

 GITHUB
  https://github.com/FirstEverTech/Universal-Intel-Chipset-Updater

 USAGE
  .\universal-intel-chipset-updater.ps1 [options]

 OPTIONS
  -help, -?          Display this help message and exit.
  -version, -v       Display the tool version and exit.
  -auto, -a          All prompts are answered with Yes,
  ...
  -skipverify, -s    Skip the script self-hash verification (sets
                     `$SkipSelfHashVerification = 1`).
                     By default verification is enabled for security;
                     use this switch only for testing.

 LOGGING
  All actions are logged to C:\ProgramData\chipset_update.log.

 EXAMPLES
  .\universal-intel-chipset-updater.ps1 -auto
  .\universal-intel-chipset-updater.ps1 -quiet
  .\universal-intel-chipset-updater.ps1 -d -s -q
...
$DebugMode = if ($Debug) { 1 } else { 0 }          # default 0 (disabled)
$SkipSelfHashVerification = if ($SkipVerification) { 1 } else { 0 }  # default 0 (enabled)
